...
# I. Introduction
- Đây là thuật toán cơ bản nhất trong nhómnhóm Unsupervised Learning.
- Ta không biết nhãn (label) của dữ liệu $\to$ nhóm các điểm dữ liệu *có cùng tính chất* vào cùng một phân cụm.

- K-means là một thuật toán phân cụm không giám sát (unsupervised learning) phổ biến với mục tiêutiêu chia tập dữ liệu gồm **$n$ điểm dữ liệu** thành **$k$ cụm** (clusters):
	- **Mỗi cụm** được biểu diễn bởi một **tâm cụm** (centroid).
	- Mục tiêu: tối thiểu hóa **phương sai trong cụm** (intra-cluster variance), nghĩa là làm giảm sự khác biệt giữa các điểm dữ liệu trong cùng một cụm.
- **Ghi chú:** Phân cụm K-means là một bài toán tối ưu hóa được xếp vào loại **NP-hard**. Để giải bài toán này một cách hiệu quả hơn, thuật toán K-means thường sử dụng các phương pháp **heuristic**, chẳng hạn như khởi tạo tâm cụm thông minh (k-means++).
# II. Phân tích toán học
- Giả sử có:
	- $N$ điểm dữ liệu: $X=[x_1, x_2, \dots , x_N] \in \mathbb{R}^{d \times N}$;
	- $K < N$ là số cluster muốn phân chia.
- Cần tìm các center (centroid) $m_1, m_2, \dots , m_K \in \mathbb{R}^{d\times 1}$ và nhãn (label) của mỗi điểm dữ liệu.
- Với mỗi điểm dữ liệu $x_i$ ta xác định vector nhãn $y_i = [y_{i1}, y_{i2}, ..., y_{ik}]$ và nếu $x_i$ có nhãn $j$ thì $y_{ij} = 1$ và còn lại là $0$. Đây gọi là kiểu biểu diễn *one-hot vector*. Ta có thể viết ràng buộc như sau: $$y_{ik} \in \{0, 1\}; \hspace{4pt} \sum_{i=1}^{K}{y_{ik}} = 1$$
...
2. **Tâm cụm (Centroid)**:
	- Tâm cụm là giá trị trung bình của tất cả các điểm dữ liệu trong cụm - vị trí trung tâm của cụm.
3. **Hàm mất mát (Objective Function)**:
...
4. **Khoảng cách Euclidean (Euclidean Distance)**:
	- Là thước đo phổ biến nhất trong K-means, được sử dụng để tính khoảng cách giữa hai điểm trong không gian.
# III. Hàm mất mát và thuật toán tối ưu
## 1. Hàm mất mát
- Nếu ta coi $m_k$ là center (hoặc đại diện -representative) của mỗi cluster:
	- *Ước lượng* tất cả các điểm được phân vào cluster này bởi ,
	- Điểm dữ liệu $x_i \in$ cluster $k$ $\to$ sai số là $(x_i−m_k)$.
		- Mong muốn sai số này có trị tuyệt đối nhỏ nhất nên ta sẽ tìm cách để đại lượng sau đây GTNN: $$\lVert x_i - m_k\lVert_2^2$$
		- Vì $x_i \in$ cluster $k$ nên trong vector nhãn one-hot có giá trị $y_{ik} = 1$ nên ta có thể viết biểu thức trên lại thành: $$\sum_{j=1}^{K}{y_{ij} \lVert x_i - m_j \lVert_2^2}$$
	- Khi đó, *sai số cho toàn bộ tập dữ liệu* tương ứng là: $$\mathcal{L}(Y, M) = \sum_{i=1}^{N} \sum_{j=1}^{K} y_{ij}\lVert x_i - m_j \lVert_2^2$$ thỏa $y_{ij}\in\{0, 1\}$ và $\displaystyle\sum_{j=1}^{K}{y_{ij} = 1}$.
## 2. Tối ưu hàm mất mát
- Bài toán trên là một bài toán khó tìm **điểm tối ưu** vì nó có thêm các điều kiện ràng buộc.
	- Thuộc loại mix-integer programming (điều kiện biến là số nguyên) - rất khó tìm nghiệm tối ưu toàn cục (global optimal point, tức nghiệm làm cho hàm mất mát đạt giá trị nhỏ nhất có thể).
...
- Bài chia nhỏ thành tìm label vector cho từng điểm dữ liệu $x_i$ như sau: $$y_i = \arg\min_{y_i} \sum_{j=1}^{K} y_{ij}\lVert x_i - m_j \lVert_2^2$$ với điều kiện đã nêu.
- Như đã nói, đây là one-hot vector nên chỉ có duy nhất một giá trị bằng 1 trong vector $y_i$, do đó ta có: $$j = \arg\min_j \lVert x_i - m_j \lVert_2^2$$
...
- Bài toán trở thành (tổng các khoảng cách min): $$m_j = \arg\min_{m_j} \sum_{i=1}^{N} y_{ij} \lVert x_i - m_j \lVert_2^2$$
...
Thuật toán K-means bao gồm các bước như sau:
...
- Giá trị của Silhouette Score nằm trong khoảng $[-1, 1]$:
    -   $1$  : gán đúng vào cụm - cụm phân tách rõ ràng.
    -   $0$  : điểm dữ liệu nằm ở biên giữa hai cụm.
    - $-1$: điểm dữ liệu có khả năng bị gán sai cụm.
## 2. Công thức
- Silhouette Score cho một điểm dữ liệu $i$ được tính như sau: $$s(i) = \frac{b(i) - a(i)}{\max(a(i), b(i))}$$trong đó:
	- $a(i)$: Khoảng cách trung bình từ điểm $i$ tới các điểm khác trong cùng cụm: $$a(i) = \frac{1}{|C_a| - 1} \sum_{j \in C_a, j \neq i} d(i, j)$$với $C_a$ là cụm mà điểm $i$ thuộc về.
	- $b(i)$: Khoảng cách trung bình từ điểm $i$ tới các điểm trong cụm gần nhất mà $i$ không thuộc về $$b(i) = \min_{C_b \neq C_a} \frac{1}{|C_b|} \sum_{j \in C_b} d(i, j)$$
...
## 3. Confusion matrix
![[attachments/Pasted image 20250106080730.png|center|500]]
